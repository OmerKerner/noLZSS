#!/usr/bin/env python3
"""
DNA factorization benchmark script for noLZSS.

This script benchmarks DNA-specific factorization functions including reverse complement
support across different input sizes. Creates log-log plots with trend lines.
...
def generate_dna_sequence(size: int) -> str:
    """
    Generate random DNA sequence.
    
    Args:
        size: Size of the sequence in nucleotides
...
def run_benchmark_suite(sizes: List[int], runs: int = 3, output_dir: str = "benchmarks/dna_results") -> Dict[str, Any]:
    """
    Run the complete benchmark suite for DNA factorization functions.
    
    Args:
        sizes: List of input sizes in nucleotides
    ...
    string_functions = {
        'factorize_dna_w_rc': cpp.factorize_dna_w_rc,
        'count_factors_dna_w_rc': cpp.count_factors_dna_w_rc,
    }
    ...
    for func_name, func in string_functions.items():
        ...
        for size in sizes:
            print(f"  Size: {size:,} nucleotides")
            
            # Generate DNA sequence
            dna_sequence = generate_dna_sequence(size)
            
            try:
                # Run benchmark
                result = benchmark_function(func, dna_sequence, runs=runs)
                if result:
    ...
    file_functions = {
        'factorize_file_dna_w_rc': (cpp.factorize_file_dna_w_rc, True),
        'count_factors_file_dna_w_rc': (cpp.count_factors_file_dna_w_rc, False),
    }
    
    for func_name, (func, needs_reserve_hint) in file_functions.items():
        print(f"\n=== Benchmarking {func_name} ===")
        results[func_name] = {'sizes': sizes, 'results': []}
        ...
        for size in sizes:
            ...
            # Generate DNA sequence and create file
            dna_sequence = generate_dna_sequence(size)
            dna_path = create_temp_file(dna_sequence)
            
            try:
                # Run benchmark with appropriate arguments
                if needs_reserve_hint:
                    result = benchmark_function(func, dna_path, 0, runs=runs)
                else:
                    result = benchmark_function(func, dna_path, runs=runs)
    ...
    print(f"\n=== Benchmarking write_factors_binary_file_dna_w_rc ===")
    results['write_factors_binary_file_dna_w_rc'] = {'sizes': sizes, 'results': []}
    ...
    for size in sizes:
        ...
        try:
            # Run benchmark
            result = benchmark_binary_function(cpp.write_factors_binary_file_dna_w_rc, dna_path, runs=runs)
            
            if result:
                result['input_size'] = size
                results['write_factors_binary_file_dna_w_rc']['results'].append(result)
...
def create_plots_and_trends(results: Dict[str, Any], output_dir: str) -> Dict[str, Dict[str, Any]]:
    ...
    fig.suptitle('DNA Factorization with Reverse Complement Benchmarks', fontsize=16)
    ...
    ax = axes[1, 1]
    if 'write_factors_binary_file_dna_w_rc' in results:
        data = results['write_factors_binary_file_dna_w_rc']
        if data['results']:
            sizes = [r['input_size'] for r in data['results']]
            ...
            if len(sizes) >= 2:
                ...
                trends["write_factors_binary_file_dna_w_rc_disk_space"] = trend
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/dna_benchmark_plots.png", dpi=300, bbox_inches='tight')
    plt.savefig(f"{output_dir}/dna_benchmark_plots.pdf", bbox_inches='tight')
    print(f"\nPlots saved to {output_dir}/dna_benchmark_plots.png and .pdf")
...
def print_summary_table(results: Dict[str, Any], trends: Dict[str, Any]):
    ...
    for func_name, data in results.items():
        ...
        print("-" * 80)
        print(f"{'Size (nt)':<15} {'Time (ms)':<12} {'Memory (MB)':<12} {'Throughput (MB/s)':<18}")
        print("-" * 80)
...
def main():
    """Main function to run the DNA factorization benchmark suite."""
    parser = argparse.ArgumentParser(description="Benchmark DNA factorization functions with trend analysis")
    parser.add_argument("--min-size", type=int, default=1000, 
                       help="Minimum input size in nucleotides (default: 1000)")
    parser.add_argument("--max-size", type=int, default=1000000,
                       help="Maximum input size in nucleotides (default: 1000000)")
    ...
    parser.add_argument("--output-dir", default="benchmarks/dna_results",
                       help="Output directory for results (default: benchmarks/dna_results)")
    ...
    print("DNA Factorization with Reverse Complement Benchmark Suite")
    print("="*50)
